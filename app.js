...
var builder = require('botbuilder'),
    ...
    recognitionService = require('./recognition-service');

// Bot Setup
var server = restify.createServer();
server.listen(process.env.port || process.env.PORT || 81,
    function() {
        ...
        if (hasImageAttachment(session)) {
            ...
            recognitionService
                ...
                .then(function (id) {
                    recognitionService
                        .extractText(id)
                        .then(function (text) { handleSuccessResponse(session, text); })
            ...
            message.membersAdded.forEach(function(identity) {
                if (identity.id === message.address.bot.id) {
                    var reply = new builder.Message()
                        ...
                        .text(
                            'Hi! I am Hand-Written Text Recognition Bot. I read a image that contains hand-writtent text. Try sending me an image or an image URL.');
                    bot.send(reply);
                }
            });
        }
...
function handleSuccessResponse(session, text) {
    if (text) {
        session.send('The text in this image is: ' + text);
    } else {
        session.send('Couldn\'t find text for this one');
    }
}
